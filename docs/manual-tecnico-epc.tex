...
Administrador & Configura parametros tarifarios, gestiona operarios & ParametrosTarifa, Admin Operarios \\
Supervisor & Consulta dashboard, valida datos & Dashboard web \\
\bottomrule
\end{tabularx}
\end{table}
...
Hardware mobile & expo-camera, react-native-ble-plx (Bluetooth impresion) \\
...
Especificacion & OpenSpec + Engram (memoria persistente) \\
...
\texttt{parametros-tarifa/} & Dominio ParametrosTarifa + flag \texttt{aplica\_cmaa} + IPC editable \\
\texttt{acuerdo-municipal/} & AcuerdoMunicipal + estado ciclo de vida \\
\texttt{suscriptores/} & Suscriptores + verificacion de estrato \\
\texttt{medidores/} & Medidores \\
\texttt{lecturas/} & Lecturas \\
\texttt{operarios/} & Operarios (admin + campo) \\
\texttt{prestadores/} & Prestadores (multi-tenant) \\
\texttt{factura/} & Factura + snapshot \texttt{validacion\_ambito} \\
...
\texttt{shared/} & Utilidades compartidas + ports \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
Res CRA 825/2017 & Prestadores rurales <5000 suscriptores & Base normativa: formula CF, CC, subsidios E1-E3, contribuciones E5-E6, CMOG minimo \\
...
\midrule
\texttt{lecturaAnterior} & number & Lectura del periodo anterior (m\textsuperscript{3}) \\
\texttt{lecturaActual} & number & Lectura del periodo actual (m\textsuperscript{3}) \\
\texttt{estrato} & Estrato (1-6) & Estrato socioeconomico del suscriptor \\
\texttt{aplicaSubsidio} & boolean & Si aplica el factor de subsidio/contribucion \\
\texttt{periodo} & \{mes, anio\} & Periodo de facturacion \\
\texttt{parametros.cargoFijo} & number & CF mensual en pesos (puede ser 0) \\
\texttt{parametros.precioM3} & number & Precio del m\textsuperscript{3} en bloque basico \\
\texttt{parametros.precioM3Excedente} & number & Precio del m\textsuperscript{3} en bloque excedente \\
\texttt{parametros.consumoBasico} & number & Limite del bloque basico (default: 20 m\textsuperscript{3}) \\
...
3 (agosto 2026) & param-tarifa-residuales-cra-825 & Flag \texttt{aplica\_cmaa} (opt-in explicito), inputs IPC editables, unificacion minimo vital Opcion A, migracion 030 aditiva idempotente, refactor num()/entero() a utils, validarTodo() modulo puro & Archivado \\
\bottomrule
\end{tabularx}
\end{table}
...
Persistencia local & SQLite (\texttt{expo-sqlite} + \texttt{better-sqlite3} en tests) \\
Persistencia servidor & PostgreSQL 16 \\
Sync & manual desde pantalla Sync, protocolo idempotente con \texttt{sync\_registros} \\
Branches & \texttt{main} + \texttt{desarrollo} + \texttt{merge-desarrollo} (cleanup pendiente) \\
